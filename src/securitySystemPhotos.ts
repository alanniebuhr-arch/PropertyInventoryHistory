import type { AppState, InventoryItem, ItemPhoto, SecuritySystemDetails } from './types';
...
function asSecuritySystemDetails(details: InventoryItem['details']): SecuritySystemDetails {
  return details.kind === 'security_system' ? details : { kind: 'security_system' };
}
...
function securitySystemSlotPhotoIds(details: SecuritySystemDetails): Set<string> {
  const ids = SECURITY_SYSTEM_PHOTO_SLOTS.map((slot) => details[slot.key]).filter(
    (id): id is string => Boolean(id)
  );
  return new Set(ids);
}

export function securitySystemExtraPhotos(
  state: AppState,
  itemId: string,
  details: SecuritySystemDetails
): ItemPhoto[] {
  const slotIds = securitySystemSlotPhotoIds(details);
  return photosForItem(state, itemId).filter((photo) => !slotIds.has(photo.id));
}

export async function addSecuritySystemExtraPhotos(
  state: AppState,
  itemId: string,
  sourceUris: string[]
): Promise<AppState> {
...
export async function removeSecuritySystemExtraPhoto(
  state: AppState,
  itemId: string,
  photoId: string
): Promise<AppState> {
...
export async function setSecuritySystemSlotPhoto(
  state: AppState,
  itemId: string,
  slotKey: SecuritySystemPhotoSlotKey,
  sourceUri: string
  ...
  nextState = await clearItemSlotDocumentOnPhotoSet(
    ...
    itemId,
    slotKey,
    asSecuritySystemDetails
  );
  const details = asSecuritySystemDetails(nextState.items.find((i) => i.id === itemId)!.details);
  const oldPhotoId = details[slotKey];

  if (oldPhotoId) {
    nextState = await clearSecuritySystemSlotPhoto(nextState, itemId, slotKey);
  }
  ...
  const currentDetails = asSecuritySystemDetails(currentItem.details);
...
export async function clearSecuritySystemSlotPhoto(
  state: AppState,
  itemId: string,
  slotKey: SecuritySystemPhotoSlotKey
): Promise<AppState> {
  ...
  const details = asSecuritySystemDetails(item.details);
...
export function updateSecuritySystemDetails(
  state: AppState,
  itemId: string,
  details: SecuritySystemDetails
): AppState {
...
export async function setSecuritySystemSlotDocument(
  state: AppState,
  itemId: string,
  slotKey: SecuritySystemPhotoSlotKey,
  sourceUri: string,
...
export async function clearSecuritySystemSlotDocument(
  state: AppState,
  itemId: string,
  slotKey: SecuritySystemPhotoSlotKey
): Promise<AppState> {
  return clearItemSlotDocument(state, itemId, slotKey, asSecuritySystemDetails);
}
