import type { AppState, InventoryItem, ItemPhoto, SolarDetails } from './types';
import { deletePhotoFile, persistPhotoFromUri } from './photoStorage';
import { photosForItem } from './storage';
import { uid, nowISO } from './utils';
import { SOLAR_PHOTO_SLOTS, type SolarPhotoSlotKey } from './solarSlots';
import {
...
function asSolarDetails(details: InventoryItem['details']): SolarDetails {
  return details.kind === 'solar' ? details : { kind: 'solar' };
}
...
function solarSlotPhotoIds(details: SolarDetails): Set<string> {
  const ids = SOLAR_PHOTO_SLOTS.map((slot) => details[slot.key]).filter(
    (id): id is string => Boolean(id)
  );
  return new Set(ids);
}

export function solarExtraPhotos(
  state: AppState,
  itemId: string,
  details: SolarDetails
): ItemPhoto[] {
  const slotIds = solarSlotPhotoIds(details);
  return photosForItem(state, itemId).filter((photo) => !slotIds.has(photo.id));
}

export async function addSolarExtraPhotos(
  state: AppState,
  itemId: string,
  sourceUris: string[]
): Promise<AppState> {
...
export async function removeSolarExtraPhoto(
  state: AppState,
  itemId: string,
  photoId: string
): Promise<AppState> {
...
export async function setSolarSlotPhoto(
  state: AppState,
  itemId: string,
  slotKey: SolarPhotoSlotKey,
  sourceUri: string
  ...
  let nextState = state;
  nextState = await clearItemSlotDocumentOnPhotoSet(nextState, itemId, slotKey, asSolarDetails);
  const details = asSolarDetails(nextState.items.find((i) => i.id === itemId)!.details);
  const oldPhotoId = details[slotKey];

  if (oldPhotoId) {
    nextState = await clearSolarSlotPhoto(nextState, itemId, slotKey);
  }
  ...
  const currentDetails = asSolarDetails(currentItem.details);
...
export async function clearSolarSlotPhoto(
  state: AppState,
  itemId: string,
  slotKey: SolarPhotoSlotKey
): Promise<AppState> {
  ...
  const details = asSolarDetails(item.details);
...
export function updateSolarDetails(
  state: AppState,
  itemId: string,
  details: SolarDetails
): AppState {
...
export async function setSolarSlotDocument(
  state: AppState,
  itemId: string,
  slotKey: SolarPhotoSlotKey,
  sourceUri: string,
...
export async function clearSolarSlotDocument(
  state: AppState,
  itemId: string,
  slotKey: SolarPhotoSlotKey
): Promise<AppState> {
  return clearItemSlotDocument(state, itemId, slotKey, asSolarDetails);
}
